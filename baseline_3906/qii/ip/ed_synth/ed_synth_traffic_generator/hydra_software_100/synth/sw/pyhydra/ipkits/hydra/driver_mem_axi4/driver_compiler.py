...
class MemAxi4Driver(Driver):
   ...
   def __init__(self, driver_idx, ip_config):
      ...
      addr_alu_arg_width   = int(params[f'DRIVER_{driver_idx}_MEM_AXI4_ADDR_ALU_ARG_WIDTH']['value'])
      ...
      awaddr_offset_width  = addr_alu_arg_width
      araddr_offset_width  = addr_alu_arg_width
      ...
      self.aw_w__addr_alu_ram = AW_W_AddrAluRam(addr_alu_arg_width)
      ...
      self.ar__addr_alu_ram   = AR_AddrAluRam(addr_alu_arg_width)
      ...
      self.b__addr_alu_ram    = B_AddrAluRam(addr_alu_arg_width)
      ...
      self.r__addr_alu_ram    = R_AddrAluRam(addr_alu_arg_width)
   ...
   def dq_alu_prbs_op(cls, arg):
      if arg == 7:
         ...
         raise ValueError(f"dq_alu_prbs_op only supports PRBS-7, 15, 23, or 31. Got unsupported value arg={arg}.")
   ...
   def addr_op(cls, start, lo, hi, align, alu):
      ...
      assert (start >= lo and start <= hi), f"start={hex(start)} must be within the boundaries of lo={hex(lo)} and hi={hex(hi)}"
      ...
      assert lo    & ((2**align) - 1) == 0, f"align={align} requires lo={hex(lo)} to be aligned to {2**align} byte boundary, i.e. {align} LSBs should be 0"
      start >>= align
      lo    >>= align
      hi    >>= align
      if alu:
   ...
   def __worker_op(cls, axid, axlen, axsize, axburst, axlock, axcache, axprot, axqos, axregion, axuser, axaddr, xdata, xstrb, xresp):
      if isinstance(axaddr, numbers.Number):
         axaddr = cls.addr_op(start=axaddr, lo=axaddr, hi=axaddr+(axlen << axsize), align=0, alu=[cls.addr_alu_echo_op()])
   ...
   def write_cmd(cls, iters, awvalid, wvalid, bready, awid, workers):
      assert iters > 0, "iters must be > 0 for write_cmd()"
   ...
   def read_cmd(cls, iters, arvalid,          rready, arid, workers):
      assert iters > 0, "iters must be > 0 for read_cmd()"
   ...
   def compile_read_write_cmd(self, cmd):
      ...
      for w in workers_orig:
         if w['axid'] in ids:
            if w['axid'] in worker_ids:
               raise ValueError(f'Workers must have unique IDs, but found multiple workers for A{RW}ID={w["axid"]}')
            workers.append(w)
            worker_ids.append(w['axid'])
         else:
            if w['axid'] not in ids_orig:
               print(f"Warning: worker for A{RW}ID={w['axid']} won't execute since its ID is not specified in a{RW.lower()}id={ids_orig}")
            else:
               print(f"Warning: worker for A{RW}ID={w['axid']} won't execute since iters={iters} doesn't cover all of a{RW.lower()}id={ids_orig}. Use iters >= {ids_orig.index(w['axid'])+1} to resolve this.")
      ...
      for w in workers:
         ...
         if w['xdata_op'] == self.OPCODE_DATA_EQ_RAW:
            ...
            if val != 0:
               data_raw = w['data_raw']
               data_actual = data_raw & ((1 << (32 * self.num_dq_alus)) - 1)
               num_dq_alus_req = math.ceil(math.log2(data_raw) / 32)
               print(f"Warning: {RW.lower()}data={hex(data_raw)} will be clipped and appear as {hex(data_actual)}. A minimum of {num_dq_alus_req} DQ ALUs are needed to represent this raw data in its entirety.")
      ...
      for w in workers:
         ...
         def gen_write_rsp_instrs():
            ...
            worker_iter_instr = self.b__worker_iter_ram.gen_instr(inf_iters_unroll0=0, iters_unroll0=worker_iters-1)
         ...
         def gen_read_rsp_instrs():
            ...
            dq_kwargs = get_worker_instr_dq_fields(w)
            dm_kwargs = get_worker_instr_dm_fields(w)
            worker_iter_instr = self.r__worker_iter_ram.gen_instr(inf_iters_unroll0=0, iters_unroll0=((w['axlen']+1)*worker_iters)-1)
   ...
   def driver_post_cmd(cls, *args):
      ...
      assert all(isinstance(i, int) for i in driver_ids), f"Found non-integer in driver_post_cmd({driver_ids}), only driver indices or Driver objects are allowed"
   ...
   def driver_wait_cmd(cls, *args):
      drivers = args[0] if isinstance(args[0], list) else args
      driver_ids = [driver.driver_idx if isinstance(driver, Driver) else driver for driver in drivers]
      assert all(isinstance(i, int) for i in driver_ids), f"Found non-integer in driver_wait_cmd({driver_ids}), only driver indices or Driver objects are allowed"
   ...
   def compile_loop_cmd(self, cmd):
      ...
      def gen_loop_instr(ax__ctrl_ram, x__ctrl_iter_ram, x__ctrl_ram, rsp_heads, cmd_head_instr_list, rsp_head_instrs_list):         
         if cmd_head_instr_list:
            ctrl_instr = ax__ctrl_ram.gen_instr(ctrl_op=1, iter_inf=iters==0, iter_num=iters-1)  
         ...
         if rsp_heads:
            for id in range(len(rsp_heads)):
               ...
               if len(rsp_head_instrs_for_this_id) > 0:
                  ...
                  ctrl_iter_instr = x__ctrl_iter_ram.gen_instr(inf_iters_unroll0=iters==0, iters_unroll0=iters*resp_iters-1)  
         ...
         for inner_cmd in inner_cmds:
            ...
            for inner_pcmd in inner_pcmds:
               if inner_pcmd.metadata[0] == "WRITE" or inner_pcmd.metadata[0] == "READ":
                  ...
                  num_ids         = self.num_wr_ids       if inner_pcmd.metadata[0] == "WRITE" else self.num_rd_ids
                  RW              = 'W'                   if inner_pcmd.metadata[0] == "WRITE" else 'R'
                  read_or_write, pwm, iters, ids, workers, = inner_pcmd.metadata[:]
                  for w in workers:
                     axid = w['axid']
                     if axid >= num_ids:
                        raise ValueError(f"A{RW}ID={hex(axid)} exceeds max value of {hex(num_ids - 1)}")
